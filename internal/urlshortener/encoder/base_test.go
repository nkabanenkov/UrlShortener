...
func TestBadAlphabet(t *testing.T) {
	enc := encoder.NewBaseEncoder([]rune{}, 0)
	if enc != nil {
...
func TestNull(t *testing.T) {
	enc := encoder.NewBaseEncoder([]rune{'a'}, 0)
	if a, err := enc.Encode(0); a != "a" || err != nil {
		t.Error("Bad encoding of 0")
	}

	enc = encoder.NewBaseEncoder([]rune{'a'}, 5)
...
func TestEncodingBinary(t *testing.T) {
	enc := encoder.NewBaseEncoder([]rune{'a', 'b'}, 0)
...
func TestEncodingOverflow(t *testing.T) {
	enc := encoder.NewBaseEncoder([]rune{'a', 'b'}, 3)
...
func TestEncodeDecode(t *testing.T) {
	enc := encoder.NewBaseEncoder([]rune{'a', 'b', 'c'}, 4)
	for i := 0; i < 3*3*3*3; i++ {
